...
case '/neko':
            ...
            case '/baka':
                ...
                case '/aesthetic':
                    ...
                    break
 case '#bokep':
if (isGroupMsg) return xbot.reply(from, 'Perintah ini hanya bisa dilakukan di chat personal!, ihsan otak bokep', id)
            if (!isPremium) return xbot.reply(from, `Fitur Ini Khusus Premium` , id)
            const list = [
                'japan',
                'bokep',
                'jav',
              ...
              axios.get(`https://api.vhtear.com/xxxsearch?query=${random}&apikey=NUSANTARA97`)
                .then(async (res) => {
                  const url = res.data.result.data[0].url
                  const play = await axios.get(`https://api.vhtear.com/xxxdownload?link=${url}&apikey=NUSANTARA97`)
                  const { title, urlVideo } = play.data.result
                  let caption = `Title: ${title}\n\n`
                  for (let i = 0; i < urlVideo.length; i++) {
                    caption += `Quality: ${urlVideo[i].quality}\nURL: ${urlVideo[i].videoUrl}\n\n`
                  }
                  await xbot.sendFileFromUrl(from, urlVideo[2].videoUrl , 'piyo.mp4' , id)
                  console.log(caption)
                })
