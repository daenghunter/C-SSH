...
module.exports = xbot = async (xbot, message) => {
    try {
        ...
        const mess = {
            wait: '_Mohon menunggu, permintaan anda sedang di proses_🕘',
            error: {
...
case '#music':
	...
	if (getaudio.data.status == false) return xbot.reply(from, getaudio.data.message, id)
	await xbot.sendFileFromUrl(from, getaudio.data.result.thumbnail, 'gambar.jpg', `Title: ${getaudio.data.result.title}\nDurasi: ${getaudio.data.result.duration}`, id)
	await xbot.sendFileFromUrl(from, getaudio.data.result.url_audio, 'uc.mp3', 'P', id)
break
        case '#play':
          //  if (!isPremium) return xbot.reply(from, `Perintah ini hanya bisa di gunakan oleh Member Premium!`, id)         
            if (!isGroupMsg) return xbot.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
